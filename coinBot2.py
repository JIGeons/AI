...
tickers = get_transaction_amount()  # 거래대금 상위 10개 코인 선정
...
while True:
    ...
    for target_ticker in tickers:
        ...
        buy_rsi14 = rsi.iloc[-3]                    # 2봉 전 RSI14  
        ...
        if money < 5000:
            ...
            # 매도 조건 충족4
            elif target_ticker == "KRW-BTC" or target_ticker == "KRW-BCH" or target_ticker == "KRW-XRP":
                if ticker_rate >= btc_revenue :
                    amount = upbit.get_balance(target_ticker)   # 현재 코인 보유 수량
                    sell_amount = amount                        # 분할 매도 비중
                    #upbit.sell_market_order(target_ticker, sell_amount)  # 시장가에 매도
                    balances = upbit.get_balances()             # 매도했으니 잔고를 최신화
                    print(f"4. sell - {target_ticker}: 가격 {amount * pyupbit.get_current_price(target_ticker)}에 {amount}개 판매, 수익률 : {ticker_rate}%")

            # 매도 조건 충족5 (3.0% 이하로 내려갈 시 손절)
                ...
                try :
                    #upbit.sell_market_order(target_ticker, sell_amount)  # 시장가에 매도
                    balances = upbit.get_balances()             # 매도했으니 잔고를 최신화
                    print(f"5. sell - {target_ticker}: 가격 {amount * pyupbit.get_current_price(target_ticker)}에 {amount}개 판매, 수익률 : {ticker_rate}%")
            ...
            # 추가 매수
                ...
                if before_rsi14 > 30 :
                    ...
                    print(f"if {buy_money} < 5000 or ({my_money - buy_money}) < 5000:")
            ...
            # 매수 조건 충족1
            if target_ticker == "KRW-BTC" or target_ticker == "KRW-BCH" or target_ticker == "KRW-XRP":
                if buy_rsi14 < 40 :
